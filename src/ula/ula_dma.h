...
#define WORDS_PER_LINE      ((PIXELS_PER_LINE + 31) / 32)   // 15 (480 bits, 24 padding)
...
typedef struct {
    uint32_t int_n[WORDS_PER_LINE];   // /INT bitstream; 1=deasserted, 0=asserted
    uint32_t clock[WORDS_PER_LINE];   // CPU clock bitstream; 1=HIGH, 0=LOW
    uint32_t pixel[WORDS_PER_LINE];   // pixel bitstream for SM1
...
extern int ch_sync;
extern int ch_pixel;
extern int ch_clock_0;   // self-chaining pair for gapless clock
extern int ch_clock_1;
extern int ch_dram;
